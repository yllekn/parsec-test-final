...
#    cpus = ['kvm', 'timing']
    ...
    # The following code is NOT the final implementation
    # There should actually be a loop to loop through all of the cpus & sizes
    
    timing_runs = starmap(createRun, product(benchmarks, ['simlarge'], ['timing'], num_cpus))
    #kvm_runs = starmap(createRun, product(benchmarks, sizes, ['kvm'], num_cpus))
    #runs = list(timing_runs) + list(kvm_runs)
    runs = list(timing_runs)
